...
export function ListsView() {
  ...
  const filtered = lists.filter((l) =>
    listsTab === "groups" ? l.isGroup : !l.isGroup
  );
      ...
      <div className="mt-5 flex flex-col gap-3 px-4">
        ...
        {filtered.length === 0 && (
          <div className="rounded-card border border-dashed border-border-strong p-6 text-center text-[14px] text-text-dim">
            No {listsTab === "groups" ? "groups" : "lists"} yet — tap the + button to add one.
          </div>
        )}
      </div>
      ...
      {showFab && (
        ...
        >
          <input
            ...
            placeholder={listsTab === "groups" ? "New group…" : "New list…"}
